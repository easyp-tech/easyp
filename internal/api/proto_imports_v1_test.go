...
func TestV1ImportsRejectInvalidSyntax(t *testing.T) {
	...
	root := t.TempDir()
	err := os.WriteFile(filepath.Join(root, "main.proto"), []byte("syntax = \"proto3\";\nimport \"missing.proto\"\n"), 0o644)
	require.NoError(t, err)

	_, err = unresolvedV1Imports(root, []string{"."})
	require.ErrorContains(t, err, "main.proto")

	_, err = v1RootImports(root, []string{"."})
	require.ErrorContains(t, err, "main.proto")
}
