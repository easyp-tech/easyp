...
func generateV1Descriptors(t *testing.T, root string, generate func(*cli.Context) error) map[string]*descriptorpb.FileDescriptorProto {
	t.Helper()
	output := filepath.Join(root, "descriptors.pb")
	flags := flag.NewFlagSet("test", flag.ContinueOnError)
	flags.String(flagGenerateDescriptorSetOut.Name, output, "")
	flags.Bool(flagGenerateIncludeImports.Name, true, "")
	ctx := cli.NewContext(&cli.App{Metadata: map[string]any{}}, flags, nil)
	ctx.Context = t.Context()
	require.NoError(t, generate(ctx))
