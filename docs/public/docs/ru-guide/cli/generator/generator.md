...
Вы можете указать плагин как массив команд для выполнения. Это полезно для запуска плагинов через `go run` без предварительной установки:
...
**Примечание:** Должен быть указан только один источник плагина (`name`, `command`, `remote` или `path`).
