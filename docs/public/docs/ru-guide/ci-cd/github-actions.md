...
2. Установлены / определены зависимости в файле `protobuf.mod` (если нужны внешние импорты).
...
- Внешние зависимые Git-клонирования (папки `vendor` или зависимости из `protobuf.mod` при использовании `easyp mod download` / `easyp mod update`).
...
- name: Cache proto deps
  ...
  with:
    path: .easyp/deps
    key: deps-${{ runner.os }}-${{ hashFiles('protobuf.mod', 'protobuf.lock') }}
    restore-keys: |
      deps-${{ runner.os }}-
```
...
Если вы указываете приватные Git-пакеты в файле `protobuf.mod`:
...
| Приватные зависимости не скачиваются | Нет SSH ключа или токена | Настроить secrets + шаг подготовки SSH |
